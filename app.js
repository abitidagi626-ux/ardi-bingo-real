...
let markedSets = []; // ለእያንዳንዱ ካርድ ማርክ መያዣ
let timeLeft = 60;
...
function openBuy(stake) {
    ...
    if(myCards.length >= 4) return;
    selectedStake = stake;
    STAKES.forEach(s => { if(s !== stake) document.getElementById(`btn-${s}`).classList.add('disabled'); });
    showPreview();
...
function showPreview() {
    ...
    grid.style.display = 'grid'; grid.style.gridTemplateColumns = 'repeat(5, 1fr)'; grid.style.gap = '2px';
...
function addCard() {
    ...
    if(myCards.length < 4) showPreview();
    else closeModal();
}
...
function startDrawing() {
    ...
    const drawIntv = setInterval(() => {
        if(idx >= 75) clearInterval(drawIntv);
        let n = pool[idx];
        drawnNumbers.add(n);
