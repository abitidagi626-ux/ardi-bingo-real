...
function init() {
    ...
    list.innerHTML = `<div style="display:grid; grid-template-columns:1fr 1fr 1fr 1fr; padding:10px; font-size:11px; color:#aaa; text-align:center; border-bottom:1px solid #004d40;">
    ...
    stakes.forEach(s => {
        ...
        row.innerHTML = `<span><b>${s}</b></span><span class="timer-display">00:60</span><span id="win-${s}">0.00</span>
            <button class="join-btn" id="btn-join-${s}" onclick="openCardSelection(${s})" style="background:#efae10; border:none; border-radius:5px; font-weight:bold; padding:5px; cursor:pointer;">Join</button>`;
...
function openCardSelection(stake) {
    if (lockStake && lockStake !== stake) return alert("You can only play in one stake per round.");
    currentStake = stake;
...
function generateCardGrid() {
    ...
    for (let i = 1; i <= 143; i++) {
        ...
        card.innerText = i;
        card.onclick = () => { if(stakeData[currentStake].bought.size < 4) showPreview(i); else alert("Max 4 Cards allowed."); };
...
function showPreview(id) {
    ...
    let lastGeneratedNums = generateBingoNumbers();
    ...
    boughtCardsNumbers["temp"] = lastGeneratedNums; 
...
function confirmPurchase() {
    ...
    playerMarkedNumbers[pendingCardId] = new Set([12]); // Auto-mark FREE
...
function startBingoArena(stake) {
    ...
    const gameInt = setInterval(() => {
        if(idx >= 75) return clearInterval(gameInt);
        let drawn = pool[idx];
...
function toggleMark(cardId, index, number, element) {
    if (index === 12) return; 
    if (!drawnNumbers.has(number)) return; // የወጣ ቁጥር ካልሆነ ክሊክ አይደረግም
...
function manualBingoCheck(cardId) {
    ...
    // ተጫዋቹ ማርክ ያደረጋቸው መስመር መስራታቸውን ማረጋገጥ
    ...
    if (isWin) {
        ...
        alert("Not Bingo! Make sure you marked a full line of drawn numbers.");
    }
}
