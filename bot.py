import logging
import random
import json
import os
from telegram import Update, KeyboardButton, ReplyKeyboardMarkup, InlineKeyboardButton, InlineKeyboardMarkup, WebAppInfo
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes, CallbackQueryHandler

# --- CONFIGURATION ---
TOKEN = "8684712579:AAE9JK0cdSK-cVeycF7xAd_KSrUUqmN5HWI"
ADMIN_ID = 1046142540
BALANCE_FILE = "balances.json"

logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)

# --- BALANCE FUNCTIONS ---
def load_balances():
    if os.path.exists(BALANCE_FILE):
        with open(BALANCE_FILE, "r") as f:
            return json.load(f)
    return {}

def save_balance(user_id, amount):
    balances = load_balances()
    user_id = str(user_id)
    balances[user_id] = balances.get(user_id, 0) + float(amount)
    with open(BALANCE_FILE, "w") as f:
        json.dump(balances, f)

def get_user_balance(user_id):
    balances = load_balances()
    return balances.get(str(user_id), 0.0)

# --- ዋናው ማስተካከያ እዚህ ጋር ነው (WebApp Data) ---
async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # ከዌብ አፑ የሚመጣውን ዳታ መቀበል
    data_raw = update.message.web_app_data.data
    user = update.effective_user
    
    # መጠኑን (Amount) ከዳታው ውስጥ መለየት (ለምሳሌ "200" ተብሎ ከመጣ)
    amount = "".join(filter(str.isdigit, data_raw)) or "0"
    
    # ለአድሚኑ የሚላኩ የማጽደቂያ ቁልፎች (Buttons)
    keyboard = [
        [
            InlineKeyboardButton("✅ Approve", callback_data=f"app_{user.id}_{amount}"),
            InlineKeyboardButton("❌ Cancel", callback_data=f"rej_{user.id}")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    admin_msg = (
        f"🔔 *አዲስ የዲፖዚት ጥያቄ*\n\n"
        f"👤 ተጠቃሚ: {user.first_name}\n"
        f"🆔 User ID: `{user.id}`\n"
        f"💰 መጠን: {amount} ETB\n"
        f"📝 ሙሉ መረጃ: {data_raw}"
    )
    
    # ለአድሚኑ ከቁልፎች ጋር መላክ
    await context.bot.send_message(
        chat_id=ADMIN_ID,
        text=admin_msg,
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
    
    # ለተጠቃሚው የተላከ መልእክት
    await update.message.reply_text("✅ የዲፖዚት መረጃዎ ለአድሚን ተልኳል። እባክዎ እስኪረጋገጥ ይጠብቁ።")

# --- አድሚኑ ቁልፉን ሲነካ የሚሰራ (Callback) ---
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    
    data_parts = query.data.split("_")
    action = data_parts[0]  # app ወይም rej
    target_user_id = data_parts[1]
    
    if action == "app":
        amount = data_parts[2]
        # ባላንስ መጨመር
        save_balance(target_user_id, amount)
        # ለተጠቃሚው ማሳወቅ
        await context.bot.send_message(
            chat_id=target_user_id, 
            text=f"🎉 የ {amount} ETB ዲፖዚት ጥያቄዎ በአድሚን ጸድቋል! ባላንስዎ ላይ ተጨምሯል።"
        )
        # የአድሚኑን መልእክት ማስተካከል
        await query.edit_message_text(text=f"{query.message.text}\n\n✅ ተቀባይነት አግኝቷል (Approved)")
    
    elif action == "rej":
        await context.bot.send_message(
            chat_id=target_user_id, 
            text="❌ ይቅርታ፣ የዲፖዚት ጥያቄዎ በአድሚን ውድቅ ተደርጓል።"
        )
        await query.edit_message_text(text=f"{query.message.text}\n\n❌ ውድቅ ተደርጓል (Cancelled)")

# --- ሌሎች የቦቱ ክፍሎች ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    contact_button = KeyboardButton("📲 ስልክ ቁጥርዎን ያጋሩ (Share Contact)", request_contact=True)
    reply_markup = ReplyKeyboardMarkup([[contact_button]], resize_keyboard=True)
    await update.message.reply_text("እንኳን ወደ Ardi Bingo በሰላም መጡ! ስልክዎን ያጋሩ።", reply_markup=reply_markup)

async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [KeyboardButton("🕹 Play Now", web_app=WebAppInfo(url=STAKE_PAGE_URL))],
        [KeyboardButton("💰 Check Balance"), KeyboardButton("💵 Deposit", web_app=WebAppInfo(url=DEPOSIT_PAGE_URL))],
        [KeyboardButton("👨‍💻 Support")]
    ]
    await update.message.reply_text("✅ ተመዝግበዋል! ከታች ያለውን አማራጭ ይጠቀሙ።", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True))

async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    user_id = update.message.from_user.id
    if text == "💰 Check Balance":
        balance = get_user_balance(user_id)
        await update.message.reply_text(f"💵 የአሁኑ ባላንስዎ፡ {balance:.2f} ETB")
    elif text == "👨‍💻 Support":
        await update.message.reply_text("አድሚኑን ያነጋግሩ፡ @ardibingobot")

def main():
    app = Application.builder().token(TOKEN).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(MessageHandler(filters.CONTACT, handle_contact))
    app.add_handler(MessageHandler(filters.StatusUpdate.WEB_APP_DATA, handle_web_app_data))
    app.add_handler(CallbackQueryHandler(admin_callback))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_messages))
    print("🚀 Bot is running with Admin Buttons...")
    app.run_polling()

if __name__ == '__main__':
    main()
