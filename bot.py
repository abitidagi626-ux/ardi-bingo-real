...
# ⚠️ አድሚን ID (ይህ ቁጥር ያንተ መሆኑን በ @userinfobot አረጋግጥ)
ADMIN_ID = 1046142540 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("እንኳን ወደ Ardi Bingo መጡ! 🎰\n\nለመመዝገብ ስልክዎን ያጋሩ።", reply_markup=reply_markup)

# 2. Handle Contact
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.message.from_user
    keyboard = [[KeyboardButton("🕹 Play Now", web_app=WebAppInfo(url="https://abitidagi626-ux.github.io/ardi-bingo-real/index.html#stake-page"))],
    ...
    await update.message.reply_text(f"✅ ምዝገባ ተጠናቋል!\n👤 ስም: {user.first_name}", reply_markup=reply_markup)

# 3. WebApp Data - እዚህ ጋር ነው ቁልፉ የሚሰራው
...
# 4. Admin Callback - አድሚኑ ሲጫነው የሚሰራ
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "💰 Check Balance":
        ...
        await update.message.reply_text(f"💵 ባላንስዎ፡ {balance:.2f} ETB")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_callback)) # ይህ መኖሩን አረጋግጥ
